...
def main(argv=None):
    ...
    while not stop_flag.is_set():
        ...
        elif in_warmup and args.auto_exposure:
            boxes = []
            mask = np.zeros_like(gray)
        else:
        ...
        if boxes and shared.detection_enabled:
            ...
            if current_candidate is None:
                ...
                if (not args.synthetic) and sp is not None and (sp < cfg.speed_px_min or sp > cfg.speed_px_max):
                    ...
                    if len(current_candidate.centers) >= cfg.frames_to_confirm:
                        if args.synthetic:
                            triggered = True
                            last_event_ts = ts
                        else:
        ...
        if shared.detection_enabled and args.synthetic and not triggered and not synthetic_forced and frame_count > 120:
            triggered = True
        ...
        produce_overlay = (cfg.preview and cv2 is not None) or (args.http_port is not None)
        overlay_frame = None
        if produce_overlay and cv2 is not None:
            ...
            if cfg.preview:
                try:
                    ...
                    cfg.preview = False
        if args.http_port is not None and overlay_frame is not None and cv2 is not None and frame_count % max(1, args.stream_every_n) == 0:
            ok, enc = cv2.imencode('.jpg', overlay_frame, [int(cv2.IMWRITE_JPEG_QUALITY), 80])
            if ok:
                with shared.lock:
                    shared.latest_jpeg = enc.tobytes()
                    # Calibration capture
                    if shared.calib_active:
